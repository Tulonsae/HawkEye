 ...
 * @author oliverw92, modified by Tulonsae
 */
public class Util {
    ...
    private static FileWriter sqlLog = null;

    /**
     * Initialize sql logger, if enabled
     * file path, specified in config, is relative data folder
     * @param plugin
     */
    public static void initSqlLog(HawkEye plugin) {
        if (Config.DbDebug) {
            File sqlLogfile = new File(plugin.getDataFolder(), Config.DbLogFile);
            try {
    ...
    /**
     * Close sql logger, if not null
     */
    public static void closeSqlLog() {
        if (sqlLog != null) {
            try {
                writeSqlLog("Ending sql logging...");
                sqlLog.close();
            } catch (IOException e) {
                log.warning("Error closing sql log file [" + Config.DbLogFile + "]: " + e);
            }
        }
    }

    /**
     * Write a message to the sql log, if enabled
     */
